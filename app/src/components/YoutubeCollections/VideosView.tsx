...
import { Card, CardMedia, CardContent, Typography, Container, Stack, Link, CircularProgress } from '@mui/material';
import { videosAtom } from '../../atoms/YoutubeCollections';
import FiltersView from './FiltersView';
import CustomGrid from './CustomGrid';

const VideosView = (props: { channelID: string | null }) => {
	const [videosView, setVideosView] = useRecoilStateLoadable(videosAtom(props.channelID));

	if (videosView.state === 'loading' || videosView.state === 'hasError') {
		return (
	...
	const { videos } = videosView.getValue();

	return (
		<React.Fragment>
			<Container sx={{ my: 5 }}>
				<FiltersView state={videosView.getValue()} updateState={setVideosView}>
					<CustomGrid
						items={videos}
						renderItem={(video) => {
